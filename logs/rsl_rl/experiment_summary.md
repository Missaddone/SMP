...
| 2026-06-24 02:08 | `smp_body_velocity_lafan_walk_posture_g1/2026-06-24_02-08-57_smp_body_velocity_lafan_walk_posture_g1` | `Smp-G1-BodyVelocity-LafanWalk-Posture-v0` | 在 LAFAN Walk posture 任务基础上扩大 command：速度 `0.15~1.6 -> 0.0~3.0 m/s`，yaw `±0.8 -> ±2.0 rad/s`；保留姿态正则 `base_upright=-0.2`、`root_height=-0.2,target_height=0.74`、`waist_pitch_roll=-0.1`、`action_rate=-0.005`；`style_floor=0.0`；用于测试更宽 command 下姿态正则是否仍能抑制弓背。 |  |
| 2026-06-24 11:37 | `smp_body_velocity_lafan_run_g1/2026-06-24_11-37-08_smp_body_velocity_lafan_run_g1` | `Smp-G1-BodyVelocity-LafanRun-v0` | 新建普通 BodyVelocity + LAFAN run prior 可行性实验；prior 使用 `lafan_run.pt`；command/reward/observation 基本沿用普通 BodyVelocity：速度 `0.0~3.0 m/s`、yaw `±2.0 rad/s`、`use_stand_branch=False`；用于排查近期异常步态是否来自 prior 训练/数据处理问题。 |  |
| 2026-06-24 12:05 | `smp_body_velocity_lafan_walk_g1/2026-06-24_12-05-48_smp_body_velocity_lafan_walk_g1` | `Smp-G1-BodyVelocity-LafanWalk-v0` | 将 LAFAN Walk 任务 prior 从 `lafan_walk_local_norm.pt` 改为新训练的小模型 `lafan_walk_500.pt`；command 保持速度 `0.15~1.6 m/s`、yaw `±0.8 rad/s`；`style_floor=0.0`；用于验证小规模 walk prior 是否可用，并排查 prior 训练/数据处理是否导致近期异常步态。 |  |
| 2026-06-24 19:21 | `smp_body_velocity_amp_run_g1/2026-06-24_19-21-55_smp_body_velocity_amp_run_g1` | `Smp-G1-BodyVelocity-AmpRun-v0` | 新建普通 BodyVelocity + AMP run prior 可行性实验；prior 使用 `amp_run.pt`；command/reward/observation 基本沿用普通 BodyVelocity：速度 `0.0~3.0 m/s`、yaw `±2.0 rad/s`、`use_stand_branch=False`；用于与 `lafan_run.pt` 对照，排查 AMP run prior 是否更适合当前 BodyVelocity 任务。 |  |
